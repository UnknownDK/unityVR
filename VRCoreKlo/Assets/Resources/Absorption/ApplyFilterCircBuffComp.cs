...
public class ApplyFilterCircBuffComp {
    ...
    public float applyFilter(float input, float[] coefficients) {
        ...
        if (coefficients.Length > 30 )
        {
            ...
            for (int i = 0; i < m; i++) 
            {
                output = output + internalBuffer2[(newBufferIndex + i) % internalBuffer2.Length]*coefficients[i];
            }
        ...
        else
        {
            ...
            for (int i = 0; i < m; i++) 
            {
                output = output + internalBuffer[(newBufferIndex + i) % internalBuffer.Length]*coefficients[i];
            }
